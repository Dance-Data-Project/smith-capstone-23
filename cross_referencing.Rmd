...
crossref %>%
  ...
  geom_label(aes(label = round(count,2), y = count, color = source),
             position = "stack",
             size = 2.6,
...
crossref_all %>%
      ...
      filter(value > 0) %>%
      group_by(EIN) %>%
      summarize(variable = paste(unique(variable), collapse=",<br>")) %>%
...
Reduce(intersect, intersections$EINs) %>% length() 
Reduce(union, intersections$EINs) %>% length() 
