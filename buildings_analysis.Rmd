...
```{r}
ggplot(buildings, aes(x = fiscal_year, y = as.numeric(TotalBookValueLandBuildingsAmt), color = as.factor(organization_name.x)))+
  theme_bw()+
  geom_line()+
  #geom_point()+
...
buildings %>% 
  ...
  theme_bw()+
  scale_color_viridis(discrete = TRUE, option="A")+
  labs(x = "Fiscal Year", y = "Sum of Total Book Values", color = "State")+
 theme(plot.title = element_text(size = 13, face = "bold", hjust = .5),
        axis.title = element_text(size = 12, face = "bold"),
        plot.subtitle = element_text(size = 5, face = "italic", hjust = .5),
        axis.text.x = element_text(size = 10, angle = 25),
...
```{r}
ggplot(building_summary,aes(x = fiscal_year, y = `Reporting companies number`))+
  geom_col()+
  theme_bw()+
  labs(x = "Fiscal Year")+
  theme(plot.title = element_text(size = 13, face = "bold", hjust = .5),
