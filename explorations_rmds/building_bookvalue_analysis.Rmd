...
```{r}
# use get_var
files<- dir("Users/niezhen/Desktop/Spring 2023/SDS 410/smith-capstone-23/data/ballet_990_released_20230208",
              full.names = TRUE)  

source(here::here("GET_VARS.R"))

buildings <- map_df(files,
                    ~get_df(filename = .x , 
                     schedule = 'd')) %>%
  filter_ein() %>% 
...
ein_to_state <- readRDS(here::here("./data/ein_to_state.RDS")) 
...
saveRDS(bookvalues,here::here('data','bookvalues.RDS'))
```
...
#ggplotly(
#  summary_state %>% 
#  ggplot(aes(x = as.factor(fiscal_year), y = as.numeric(median), color = as.factor(state))) + 
#    geom_line(aes(group=1)) + 
#    theme_bw()) +
#  labs(
