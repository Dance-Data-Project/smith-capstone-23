...
add <- map_df(files,
              ...
              names = c('name', 'type_code', 'business_state_cd', 'books_state_cd', 'books_state'))) %>%
  filter_ein()
...
# number that did not report for any year on file
no_business_cd <- add %>%
  mutate(name = tolower(name)) %>%
  group_by(EIN) %>%
  summarize(business_state_cd = list(unique(na.omit(business_state_cd))),
            num_states = map_dbl(business_state_cd, length),
...
However, are `r no_business_cd %>% nrow()` states that do not report the business state code in any year on file. These companies are:

`r no_business_cd %>% pull(name) %>% paste(collapse = "<br>")`.
...
address_vars <- tibble(varname = get_all_children(file_to_check, "//ReturnData")) %>%
  filter(grepl("saddress", varname, ignore.case =TRUE)) %>%
  pull(varname)
