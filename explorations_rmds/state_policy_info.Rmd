...
add <- map_df(files,
              ...
              names = c('type_code', 'business_state_cd', 'books_state_cd', 'books_state'))) %>%
  filter_ein()
...
# did companies report in some years but not others?
# 78 did not report business_state_cd in any year on file
add %>%
  select(EIN, fiscal_year, business_state_cd) %>%
  group_by(EIN) %>%
  mutate(num_nas = sum(is.na(business_state_cd)),
         total = n()) %>%
  filter(num_nas == total)

```

The ideal classification of state is when we have the Business Address `IRS990/USAddress/StateAbbreviationCd`.

However, `r add %>% filter(is.na(business_state_cd)) %>% pull(EIN) %>% unique() %>% length()` do not report this variable. For these companies, we set the state to where the company has their bookings, taht is,  `BooksInCareOfDetail/USAddress/StateAbbreviationCd`.



`r add %>% group_by(EIN) %>% 
  mutate(states = n_distinct(state)) %>% 
  filter(states>1) %>% pull(EIN) %>% unique() %>% length()` companies changed in geographic state:

* EIN `454864757` (Dissonance Dance Theatre)  did not have a business state in 2015-2016, but had bookings in UT in 2017. From 2018 onward, the business state was in DC.
* EIN `813872831` (Ballet Sun Valley) had the business state CA in 2017-2019 but changed to ID in 2022.


```{r}

# two EINs had a change in business
add %>% group_by(EIN) %>% 
  mutate(states = n_distinct(state)) %>% 
  filter(states>1) %>%
