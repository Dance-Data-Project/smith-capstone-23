...
```{r}
companies <- read_csv("/Users/niezhen/Desktop/Spring 2023/SDS 410/smith-capstone-23/data/companies.csv")
...
bookvalues<-bookvalues %>%
  ...
  arrange(desc(EIN)) %>% 
  left_join(y = companies %>% rename(EIN = ein) %>% mutate(EIN = as.character(EIN)) , by = "EIN") %>% 
  left_join(y = ein_to_state, by = "EIN")
...
# high
high_graph<-ggplot(high, aes(x = year, y = BookValueSum, color = as.factor(name))) +
  geom_line() +
  theme_bw() +
  theme(plot.title = element_text(size = 10, face = "bold", hjust = .5),
        axis.title = element_text(size = 10, face = "bold"),
...
ggplotly(
  summary_state %>% 
  ggplot(aes(x = fiscal_year, y = max, color = factor(name))) + geom_point()+ theme_bw()
)
```
...
```{r}
ggplotly(
  test_state %>% group_by(state, fiscal_year) %>% 
  summarize(
    max = max(TotalBookValueLandBuildingsAmt),
    min = min(TotalBookValueLandBuildingsAmt),
    mean = mean(TotalBookValueLandBuildingsAmt),
...
```{r}
# companies whose business state is different from books state
diff_state<-test_state %>% 
 filter(business_state_cd != books_state_cd)
diff_state %>% 
  select(EIN, fiscal_year, name, business_state_cd, books_state_cd, state)
```
