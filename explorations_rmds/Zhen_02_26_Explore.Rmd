...
```{r copied from "data_dictionary" to get the variables needed for the next chunk, eval = FALSE}
################################################################################
...
files <- dir(here("ballet_990_released_20230208"), full.names = TRUE)
...
## Adding Building related variables to endowment data

```{r all the variables to add to the meta dataframe, eval = FALSE}
...
```{r}
#all_data_filtered<-readRDS(here("data", "endowment_filter_data_990.RDS"))
...
## Buildings Group Variable Explanation
...
saveRDS(endowment_data_building, "./endowment_filtered_buildings.RDS")

saveRDS(all_data_filtered, "./all_data_filtered.RDS")
```
...
buildings <- map_df(files, ~get_df(filename = .x , 
                              schedule = 'd')) %>%
  filter_ein() %>% 
  select(ReturnTs, EIN,OtherLandBuildingsGrp,TotalBookValueLandBuildingsAmt,BuildingsGrp)
```
...
#change this to FY instead (use get_var)
