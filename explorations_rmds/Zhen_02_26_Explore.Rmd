...
# filter columns related to buildings and land
cols <- names(schedule_d)
index <- grepl("Land|Building|EIN|ReturnTs", cols)
cols <- cols[index]

# remove /Return/ReturnData/IRS990ScheduleD/ from columns since this is shared across all columns
schedule_d <- subset(schedule_d, select = cols) %>% 
...
```{r}
# load in company names corresponding to EIN
companies <- read_csv(here("data/companies.csv")) %>%
  mutate(EIN = as.character(ein))

#load in regional information
ein_to_state <- readRDS(here("./data/ein_to_state_update.RDS")) 

# joining these information to scheudle_d
schedule_d_buildings_lands_fy<- schedule_d %>% 
  left_join(select(endow, EIN, fiscal_year), by = "EIN") %>% 
  left_join(companies, by = "EIN") %>% 
  left_join(ein_to_state, by = "EIN") %>% 
  #select(-c("OtherLandBuildingsGrp", "BuildingsGrp","LandGrp", "fiscal_year.y", "ein","organization_name.y")) %>% 
  unique() %>% 
  group_by(EIN) %>% 
  arrange(desc(ReturnTs))

schedule_d_buildings_lands_rt<- schedule_d %>% 
  left_join(companies, by = "EIN") %>% 
  left_join(ein_to_state, by = "EIN") %>% 
  #select(-c("OtherLandBuildingsGrp", "BuildingsGrp","LandGrp", "fiscal_year.y", "ein","organization_name.y")) %>% 
  unique() %>% 
...
saveRDS(schedule_d_buildings_lands_fy, here("data","schedule_d_buildings_lands_fy.RDS"))
saveRDS(schedule_d_buildings_lands_rt, here("data","schedule_d_buildings_lands_rt.RDS"))
```
## Analyze using only ReturnTs
```{r}
# 
data_each_year_rt<-list(
  data_2015 <- filter(schedule_d_buildings_lands_rt, grepl("2015", ReturnTs)),
  data_2016 <- filter(schedule_d_buildings_lands_rt, grepl("2016", ReturnTs)),
  data_2017 <- filter(schedule_d_buildings_lands_rt, grepl("2017", ReturnTs)),
  data_2018 <- filter(schedule_d_buildings_lands_rt, grepl("2018", ReturnTs)),
  data_2019 <- filter(schedule_d_buildings_lands_rt, grepl("2019", ReturnTs)),
  data_2020 <- filter(schedule_d_buildings_lands_rt, grepl("2020", ReturnTs)),
  data_2021 <- filter(schedule_d_buildings_lands_rt, grepl("2021", ReturnTs))
)

# get the most updated values of each fiscal_year according to ReturnT
for(i in 1:length(data_each_year_rt)){
  data_each_year_rt[[i]] <- data_each_year[[i]] %>% 
    group_by(organization_name.x,ReturnTs) %>% 
    filter(ReturnTs == max(as.Date(ReturnTs)))
}

# buildings_updated_values_rt <- bind_rows(data_each_year)

BookValueSum <- list()

# restore each year's sum for each EIN's buildings bookvalue 
for(i in c(1:7)){
  data = data_each_year_rt[[i]]
  df <- data.frame(data %>% 
...
BookValueSum <- bind_rows(BookValueSum) %>% 
  mutate(EIN = as.character(EIN)) %>% 
  left_join(companies, by = "EIN") %>% 
  left_join(ein_to_state, by = "EIN")
...
data_each_year<-list(
  data_2015 <- filter(schedule_d_buildings_lands, grepl("2015", fiscal_year)),
  data_2016 <- filter(schedule_d_buildings_lands, grepl("2016", fiscal_year)),
  data_2017 <- filter(schedule_d_buildings_lands, grepl("2017", fiscal_year)),
  data_2018 <- filter(schedule_d_buildings_lands, grepl("2018", fiscal_year)),
  data_2019 <- filter(schedule_d_buildings_lands, grepl("2019", fiscal_year)),
  data_2020 <- filter(schedule_d_buildings_lands, grepl("2020", fiscal_year)),
  data_2021 <- filter(schedule_d_buildings_lands, grepl("2021", fiscal_year))
)
```
...
```{r}
graphs <- list()
for(i in 1:length(data_each_year)){
  data_each_year[[i]]<-data_each_year[[i]] %>% 
    mutate(ReturnTs = as.Date(ReturnTs)) %>% 
    arrange(ReturnTs)
  

    graphs[[i]] <- ggplot(data_each_year[[i]], aes(x = ReturnTs, y = as.numeric(TotalBookValueLandBuildingsAmt), color = organization_name.x))+
...
for(i in 1:length(data_each_year)){
  data_each_year[[i]] <- data_each_year[[i]] %>% 
    group_by(organization_name.x,fiscal_year) %>% 
    filter(ReturnTs == max(as.Date(ReturnTs)))
}

buildings_updated_values <- bind_rows(data_each_year)
```

## TotalBookValueLandBuildingsAmt
```{r}
ggplotly(
  ggplot(buildings, aes(x = fiscal_year, y = as.numeric(buildings$TotalBookValueLandBuildingsAmt), color = organization_name.x))+
  geom_point()
)
```
...
```{r}
ggplotly(
  ggplot(buildings, aes(x = fiscal_year, y = as.numeric(buildings$`OtherLandBuildingsGrp/DepreciationAmt`), color = organization_name.x))+
  geom_line()
)
```
 
## Number of companies updating Buildings and Land filings for each fiscal year
```{r}
# 
schedule_d_buildings_lands %>% 
  group_by(fiscal_year, organization_name.x) %>% 
  summarise(number = n()) %>% 
  arrange(desc(number))
```
