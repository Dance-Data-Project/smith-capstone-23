...
ruthdata %>% 
  group_by(EIN, year) %>% 
  mutate(
    rank_vol_pct = rank(desc(volunteer_pct))
  )
```
...
```{r}
# under construction
ruthdata$vol_pct_quantile <- cut(ruthdata$volunteer_pct, breaks = quantile(ruthdata$volunteer_pct, na.rm = TRUE), labels = FALSE)
  
options(digits = 4)
regions_num <- ruthdata %>% 
  group_by(region) %>% 
  summarise(number = n())

quantile_num <- ruthdata %>% 
  group_by(vol_pct_quantile) %>% 
  summarise(quant_num = n())

stack_region<-ruthdata %>% 
  group_by(region, vol_pct_quantile) %>% 
  summarise(number = n()) %>% 
  left_join(regions_num, by = "region") %>% 
  mutate(Percent = number.x/number.y * 100)

stack_quantile <- ruthdata %>% 
  group_by(vol_pct_quantile, region) %>% 
  summarise(number = n()) %>% 
  left_join(quantile_num, by = "vol_pct_quantile") %>% 
  mutate(Percent = number/quant_num * 100)
```

```{r}
ggplot(stack_region, aes(x = region, y = Percent, fill = vol_pct_quantile)) +
...
```{r}
ggplot(stack_quantile, aes(x = vol_pct_quantile, y = Percent, fill = region)) +
  geom_bar(stat = "identity", position = "stack")+
  viridis::scale_fill_viridis(discrete=TRUE,
                                 option = "rocket",
                                 end = .9)+
  theme_bw()+
  xlab("Quantile of Percentage of Volunteer in Total Labor Count")
