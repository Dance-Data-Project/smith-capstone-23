...
```{r, fig.height = 17, fig.cap = "\\label{fig:annual-growth-endowment}Annual growth rate of a company's endowment when adjusting for contributions and withdrawals, compared to the annual growth S\\&P 500 for the corresponding time period."}
...
# dat %>%
#   mutate(color_organization = ifelse(
#     organization_name %in% top$organization_name, 
...
#   geom_line(show.legend=FALSE,
#             alpha = .1) +
#   geom_line(alpha=1, aes(color = color_organization)) +
#    geom_point(size=.5, alpha =.5,
#                aes(color = color_organization)) + 
#   facet_wrap(~name) +
#   scale_color_manual(values=pal,
#                      na.value=NA, 
#                      breaks =top$organization_name) 
...
#   
  ...
  #  scale_x_continuous(limits=c(2012.5,2021.5), breaks=2014:2021) +
...
```{r, eval=TRUE, fig.width = 12, fig.height=8.5, fig.cap="\\label{fig:frac-comp} percent of the total compensation paid to employees that was paid to officers, directors, trustees, or key employees, as reported in Part IX of the Form 990. Highlighted in the first panel are the 10 companies that had the greatest change in the percent paid to C-Suite employees from the earliest year on file to the latest. Only companies with more than 45 employees and that reported complete data for more than 5 years are included."}
...
# emp_comp %>%
#   group_by(organization_name) %>%
#   mutate(reported_offcomp = ifelse(is.na(OffDirCompAmt) | OffDirCompAmt == 0, "No", "Yes")) %>%
#   group_by(organization_name,reported_offcomp) %>%
#   summarize(n=n()) %>%
#   pivot_wider(values_from=n, names_from=reported_offcomp) %>%
#   filter(is.na(Yes))
...
pal1 <-viridis_pal(option="mako", end = .8, begin=.2)(4)
pal2 <- viridis_pal(option="inferno", end = .8, begin=.2)(3)
pal3 <- viridis_pal(option="magma", end = .8, begin=.2)(3)
...
##################################
plt1 <- emp_comp %>%
  # filter(org_size != "Less than 45 Employees") %>%
  ...
  geom_text(x=2020.2, y = .1, label = "Median",color="darkred") +
...
```{r, fig.width=10,fig.height=5.5, fig.cap = "\\label{fig:csuite-comp-bar}"}
...
emp_comp %>%
  ...
  group_by(organization_name) %>%
   ggplot(aes(x=fiscal_year,
             ...
             color = organization_name)) +
  geom_point(size =.6, alpha=.5) +
  geom_line(show.legend=FALSE, alpha = .5)+
     facet_wrap(~is_top) +
  theme_c() 
...
plt1 <- emp_comp %>%
  mutate(top = ifelse(organization_name %in% top$organization_name,
         organization_name, NA),
         is_top = ifelse(
           !is.na(top), 
           "5 Companies with Greatest Change\nin Fraction Paid to C-Suite",
           "Other Companies")) %>%
  filter(!is.na(top)) %>%
  ggplot(aes(x=fiscal_year,
             y = frac,
  ...
  geom_point(size =.6, alpha=.5) +
  geom_line(show.legend=FALSE, alpha = .5)+
  geom_line(aes(color=top), linewidth=1.2)  +
  theme_bw() +
  labs(
    # y="percent of Total Compensation Paid",
    #   title = "percent of Total Compensation Paid to C-Suite Employees",
       x="Fiscal Year",
       y="percent",
       color = "") +
   # scale_y_continuous(n.breaks = 6, limits = c(.04,.3), labels=scales::percent)+
    theme(plot.title = element_text(size = 14,
                                      hjust = .5, face="bold",),
            plot.subtitle = element_text(hjust = .5,
                                         face="italic",
            ...
            axis.title = element_text(size = 13,
                                      face = "bold")) +
  scale_color_manual(values=pal,na.value = NA, breaks = top$organization_name,
                     labels = top$organization_name) +
...
pal1 <-viridis_pal(option="mako", end = .8, begin=.3)(8)
pal2 <- viridis_pal(option="inferno", end = .8, begin=.3)(6)
pal3 <- viridis_pal(option="magma", end = .8, begin=.1)(6)
...
plt2 <- emp_comp %>%
  group_by(organization_name) %>%
  mutate(has_data = any(!is.na(frac))) %>%
  filter(has_data) %>%
  ungroup() %>%
  mutate(top = ifelse(organization_name %in% top$organization_name,
         organization_name, NA),
  ...
  ggplot(aes(x=fiscal_year,
             ...
             group= organization_name,
             color = organization_name)) +
  geom_point(size =.6, alpha=.5, show.legend=FALSE) +
  geom_line(show.legend=FALSE)+
  facet_wrap(~is_top) +
  theme_bw() +
  labs(
