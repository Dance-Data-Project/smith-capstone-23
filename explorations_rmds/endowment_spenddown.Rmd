...
low_forty <- pct_change_inv_rtd %>%
    filter(pct_change < -40) %>%
    select(organization_name, TaxPeriodEndDt,pct_change, fiscal_year)

minimums_forty <- pct_change_inv_rtd %>%
  group_by(organization_name) %>%
  summarize(pct_change = min(pct_change)) %>%
  ungroup() %>%
  filter(pct_change < -40) %>%
  left_join(pct_change_inv_rtd, by = c("organization_name", "pct_change")) %>%
  select(organization_name, TaxPeriodEndDt,pct_change, fiscal_year)

##Plotting and labeling only those with dips below 40
pct_change_inv_rtd %>%
  filter(pct_change != Inf) %>%
  filter(organization_name %in% minimums_forty$organization_name) %>%
  ggplot(aes(x = TaxPeriodEndDt, y = pct_change)) +
...
high_200 <- pct_change_inv_rtd %>%
    filter(pct_change >200) %>% 
  filter(pct_change != Inf) %>%
    select(organization_name, TaxPeriodEndDt,pct_change, fiscal_year) 

max_200 <- pct_change_inv_rtd %>%
  group_by(organization_name) %>%
  summarize(pct_change = max(pct_change)) %>%
  ungroup() %>%
  filter(pct_change >= 200) %>%
  left_join(pct_change_inv_rtd, by = c("organization_name", "pct_change")) %>%
  select(organization_name, TaxPeriodEndDt,pct_change)

##Plotting and labeling only those with dips below 40
pct_change_inv_rtd %>%
  filter(pct_change != Inf) %>%
  filter(organization_name %in% max_200$organization_name) %>%
  ggplot(aes(x = TaxPeriodEndDt, y = pct_change)) +
  geom_line(aes(group = organization_name, color = organization_name), show.legend = TRUE) + 
  #scale_colour_manual(values = pal) + 
  scale_color_viridis_d(option = "D") +
...
inv_consist_ds <- inv_consist %>% 
  ...
  right_join(pct_change_inv_rtd, by = "organization_name") %>%
  filter(pct_change != Inf & pct_change != -Inf)
...
ggplot(inv_consist_ds, aes(x = TaxPeriodEndDt, y = pct_change, 
