...
pct_change_inv_rtd %>%
  filter(pct_change != Inf) %>%
  filter(organization_name %in% minimums$organization_name) %>%
  ggplot(aes(x = TaxPeriodEndDt, y = pct_change)) +
  geom_line(aes(group = organization_name, color = organization_name), alpha = 0.5, show.legend = TRUE) + 
  #scale_colour_manual(values = pal) + 
