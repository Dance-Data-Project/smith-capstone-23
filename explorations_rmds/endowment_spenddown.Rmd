...
most_recent_ds %>%
  ...
  filter(!is.na(TotalEmployeeCnt)) %>%
  select(organization_name, TotalEmployeeCnt, EndYearBalanceAmt, recent_year) %>%
  make_table("Size of Inconsistent Companies, by Employees and Endowment", col_names = c("Organization Name", "Total Employee Count", "End Year Balance Amount", "YFiscal ear")) %>%
  scroll_box(height = "450px")
```
