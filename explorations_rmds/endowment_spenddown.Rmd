...
sp_pct_chg <- sp_begin %>%
  ...
  mutate(pct_change_mon = (sp500_END - sp500_START)/sp500_START * 100)
```
...
pct_change_rtd %>%
  ...
  geom_point(data = pct_change_med, aes(x = fiscal_year, y = median), color = "black", size = 1) + 
  geom_line(data = sp_pct_chg, aes(x = date_END, y = pct_change_mon), color = "black", show.legend = FALSE)
```
...
sp_change_years <- sp_end %>%
  left_join(sp_start, by = c("month", "year_last" = "year")) %>%
...
pct_change_rtd %>%
  ...
  scale_x_date(breaks = scales::pretty_breaks(n = 20)) +
  geom_point(data = pct_change_med, aes(x = fiscal_year, y = median), color = "black", size = 1) + 
  geom_line(data = sp_change_years, aes(x = date_end, y = pct_change_yr), color = "black", show.legend = FALSE, alpha = 0.5)
```
...
## Looking at Companies who Drop at Any Point 
