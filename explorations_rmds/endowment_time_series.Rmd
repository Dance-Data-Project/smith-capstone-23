...
plotlist <- pmap(variable_combinations, ~{ 
 #plt <- plot_ranks(var1 = .x, var2 = .y, data = endowment_data_wide)
   plt <- plot_ranks_by_consistency(var1 = .x, var2 = .y, data = endowment_data_wide)
...
plotlist <- map(vars,~ {
  plt<- plot_ranks_over_time(endowment_data_wide,.x)
})
...
```{r, eval=TRUE, results='asis', fig.height = 8, fig.width = 12}
...
plt <-  emp_comp %>%
 ...
 ranks <- emp_comp %>%
   ...
   mutate(category = case_when(sum_pos != 0 & sum_neg != 0 ~ "Had Some Change",
                               sum_zero ==  sum_pos + sum_neg + sum_zero ~ "Always Ranked the Same",
                            sum_pos == sum_pos + sum_neg + sum_zero ~  "Always Ranked Higher in Average Employee Compensation" ,
                            sum_neg == sum_pos + sum_neg + sum_zero ~ "Always Ranked Higher in C-Suite Compensation"),
...
plt <- ranks %>%
   ...
   labs(y = "Ranking of Average Employee Compensation",
        ...
        color = "Proportion Where Average Employee Compensation\nWas Ranked Higher Than C-Suite") +
   facet_wrap(~fiscal_year)  +
  geom_function(fun=function(x)x,color="darkred", alpha = .8) +
  scale_color_distiller(palette ="PuOr")
...
A percentage change of 1 means the value in the current year is double that of hte previous, since we have $\text{Value Current Year} = (1+1) \times \text{Value in the Previous Year}$.
...
```{r, eval=TRUE,  results='asis', fig.height =14, out.width="90%"}
...
```{r, eval=TRUE, fig.height = 13, fig.width = 18}
