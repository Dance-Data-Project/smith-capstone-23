...
# generates vector of the full paths to all files in the archive directory
files <- dir( Sys.getenv("ARCHIVE_NAME"), full.names = TRUE)
...
all_data <- all_data %>%
  mutate(ReturnDate = as_date(ReturnTs,
                              format = "%Y-%m-%d")) %>%
  mutate(across(c(TaxPeriodBeginDt, 
                  TaxPeriodEndDt),
                \(x) as_date(x, format = "%Y-%m-%d"))) %>%
