...
filed_schd_d %>%
 ...
 # mutate(type = factor(type, levels = "Schedule D Filings", "990 Filings")) %>%
  ggplot(aes(xmin = as.Date(TaxPeriodBeginDt, "%Y-%m-%d"), 
             xmax = TaxPeriodEndDt, 
             y = EIN,
             color = type)) +
...
```
> 943417247, 931009305, 846038137, 840776736, 746050356, 550397803, 210732575 probably interesting to investigate  
...
filed_schd_d %>%
  ...
  filter(gap > 1) %>%
  select(EIN, gap, before, TaxPeriodBeginDt) %>%
  make_table(title = "Schedule D Filing Gaps", col_names = c("EIN", "Gap Length", "Gap Start Date", "Gap End Date"))
```

```{r}
#Retrieving list of 990 filing gaps
filed_schd_d %>%
  filter(EIN %in% ein_with_sch_d) %>%
  select(EIN, 
         TaxPeriodBeginDt, 
  ...
  filter(gap > 1) %>%
  select(EIN, gap, before, TaxPeriodBeginDt) %>%
  make_table(title = "990 Filing Gaps for EINs\nwhich once filed Schedule D", col_names = c("EIN", "Gap Length", "Gap Start Date", "Gap End Date"))
```
