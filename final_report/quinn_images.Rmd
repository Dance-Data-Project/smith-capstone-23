...
endowment_data <- read_rds(here(
  "data", 
  "endowments_by_most_recent_filings.RDS")) %>%
# taking the absolute value to handle (most) described discrepancies 
# where calculated end of year balance does not match the reported
  mutate(across(c(AdministrativeExpensesAmt,
                  ContributionsAmt,OtherExpendituresAmt ), abs)) %>%
  select(-c(EndowmentsHeldUnrelatedOrgInd, 
...
```{r, fig.height = 19, fig.cap = "\\label{fig:annual-growth-endowment}Annual growth rate of a company's endowment when adjusting for contributions and withdrawals, compared to the annual growth S\\&P 500 for the corresponding time period.", fig.pos="h"}
...
end_sp %>%
  ...
  facet_wrap(~organization_name, ncol=4) +
  theme_c() +
