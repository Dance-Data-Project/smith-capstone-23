...
```{r copied from "data_dictionary" to get the variables needed for the next chunk}
################################################################################
...
changes <- bookvalues %>% 
  ...
  select(EIN, BusinessName.x, year, BookValueSum, change) %>% 
...
```{r}
all_changes_pct <- ggplot(changes, aes(x = year, y = changes$change, color = as.factor(changes$BusinessName.x))) +
...
filtered_changes <- changes %>% 
  ...
  ggplot(aes(x = year, y = change, color = as.factor(EIN))) +
  geom_line(na.rm = FALSE, size = 0.2) +
  ylim(-200, 800)+
  theme_bw() +
  theme(legend.key.size = unit(1, 'mm'),
...
ranking<-changes %>%
  ...
  select(EIN, BusinessName.x, year, change, BookValueSum) %>% 
  kbl(caption = "Ranking of Single Book Value change (%)") %>%
  kable_material() %>%
  row_spec(row = 0, background = "red", color = "white", bold = TRUE)
```
